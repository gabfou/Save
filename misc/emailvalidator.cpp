...
void emailvalidator::send()
{
    ...
    if (ref)
        qry.prepare("UPDATE project_" + m->namecurrent + "_project SET refbool = refbool + 1 WHERE " + strallgroupfilsforsql(groupid, m->namecurrent) + ";");
    else
         qry.prepare( "UPDATE project_" + m->namecurrent + "_project SET questionbool = questionbool + 1 WHERE " + strallgroupfilsforsql(groupid, m->namecurrent) + ";");
    if(!qry.exec() )
