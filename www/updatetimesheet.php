...
try{
	$req_pre = $bdd->prepare('SELECT questionbool FROM project_'.htmlspecialchars($_SESSION['project']).'_project WHERE id = '.htmlspecialchars($_SESSION['id_client']).";"); // changer user		  
	$req_pre->execute();
	}
...
foreach ($tab as $name)
{
	...
	if (isset($_POST[$name.'#note']))
		$qry = 'INSERT INTO project_'.htmlspecialchars($_SESSION['project']).'_reponse (idperson , name, time, note, date_info, iteration) VALUE ('.htmlspecialchars($_SESSION['id_client']).' , "'.$name.'" , '.htmlspecialchars($_POST[$name.'#time']).', '.htmlspecialchars($_POST[$name.'#note']).' , NOW() , 1 );';
	else
		$qry = 'INSERT INTO project_'.htmlspecialchars($_SESSION['project']).'_reponse (idperson , name, time, note, date_info, iteration) VALUE ('.htmlspecialchars($_SESSION['id_client']).' , "'.$name.'" , '.htmlspecialchars($_POST[$name.'#time']).', '.'-1'.' , NOW() , 1 );';
	$bdd->exec($qry);
