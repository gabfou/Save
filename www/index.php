		...
		while ($groupid > -1)
		{
			...
			foreach ($tabquestion as $key => $value)
			{
				if ($value['ref_only'] != 1)
				{
					...
					{
						$echofinal = $echofinal.'<label class="label" id="lab'.$value['question'].'" for "'.$value['question'].'#time"> '.$value['sujet'].': </label><select class = "reponse" id="rep'.$value['question'].'" name="'.$value['question'].'#time" >';
						$select = explode(" ", $value['splitchar']);
						foreach ($select as $selectkey => $selectvalue)
						{
							$echofinal = $echofinal.'<option value = '.$selectvalue.' name="'.$value['question'].'#time">'.$selectvalue.'</option>';
						}
