<?php
	...
	{
		echo "<html>ca ne marche pas</html>";
		exit;
	}
	$firstname = "'prenoms'";
	$lastname = "'noms'";
	$questionbool = $_GET['questionbool'];
	$refbool = $_GET['refbool'];	
	$groupid = $_GET['groupid'];
?>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<head>
	<link rel="stylesheet" href="style.css" />
	<title><?php echo htmlspecialchars($_GET['project']);?></title>
	<!-- <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/0.97.7/css/materialize.min.css"> -->
</head>
<body>
	<div class="topbar">
		<a href="index.php"><img src="logonew.png" alt="logo murano" class = logo></a>
		<h1 class = titre><?php echo str_replace("_", " ", htmlspecialchars($_GET['project'])); ?></h1>
...
<?php
	...
	if (isset($_GET['project']))
	{
		...
		while ($groupid > -1)
		{
			$req_pre = $bdd->prepare('SELECT id, question, note, type, sujet, qgroupid, typef, ref_only, splitchar FROM project_'.htmlspecialchars($_GET['project']).'_question WHERE groupid = '.htmlspecialchars($groupid).";"); // changer user
			$req_pre->execute();
			($tabquestion = $req_pre->fetchall());
			foreach ($tabquestion as $key => $value)
			{
				if ($value['ref_only'] == 0
					|| ($_GET['iteration'] == 0  && $value['ref_only'] == 1)
					|| ($_GET['iteration'] != 0 && $value['ref_only'] == 2))
				{
					$req_pre2 = $bdd->prepare('SELECT gquestion FROM project_'.htmlspecialchars($_GET['project']).'_groupe WHERE id= '.$value['qgroupid'].";");
					$req_pre2->execute();
					$tabg = $req_pre2->fetch();
					$echopargroupe[$value['qgroupid']] = $echopargroupe[$value['qgroupid']].'<div>';

					$req_pre2 = $bdd->prepare('SELECT time, str FROM project_'.htmlspecialchars($_GET['project']).'_reponse WHERE idperson = 0 AND iteration = '.htmlspecialchars($_GET['iteration']).' AND idquestion = '.htmlspecialchars($value['id']).';'); // changer user
					$req_pre2->execute();
					$rep = $req_pre2->fetch();
					...
					if ($value['typef'] == 0)
					{
						...
						if (!isset($minmax[0]))
							$minmax[0] = '0';
						if ($_GET['iteration'] == 0 && $value['type'] === "min" && $sujet === "Time spent")
						{							
							$type = "%";
					...
					if ($_GET['iteration'] == 0 && $value['note'])
			...
			$req_pre = $bdd->prepare('SELECT groupname, groupparent FROM project_'.htmlspecialchars($_GET['project']).'_groupe WHERE id = '.$groupid.";"); // changer user
			$req_pre->execute();
			($tab = $req_pre->fetch());
			if ($tab === false)
				$groupid = -1;
		...
		if ($_GET['iteration'] == 0)
			$req_pre = $bdd->prepare('SELECT introref FROM all_config WHERE project_name = "'.htmlspecialchars($_GET['project']).'";');
		else
			$req_pre = $bdd->prepare('SELECT introreel FROM all_config WHERE project_name = "'.htmlspecialchars($_GET['project']).'";');
		$req_pre->execute();
		($tab = $req_pre->fetch());
		if ($tab['introreel'] != "" && $_GET['iteration'] != 0)
			$echofinal = $echofinal.'<div id = "intro">'.$tab['introreel'].'</div>';
		else if ($tab['introref'] != "" && $_GET['iteration'] == 0)
		...
		$echofinal = str_replace("__%j__", $_GET['iteration'], $echofinal);
		...
		while ($nop > 0)
		{
			...
			{
				$req_pre = $bdd->prepare('SELECT groupname, description, groupparent, gquestion FROM project_'.htmlspecialchars($_GET['project']).'_groupe WHERE id= '.htmlspecialchars($value).";"); 
				$req_pre->execute();
				$str3 = $req_pre->fetch();
				$req_pre = $bdd->prepare('SELECT id FROM project_'.htmlspecialchars($_GET['project']).'_question WHERE qgroupid= '.htmlspecialchars($value).";"); 
				$req_pre->execute();
		...
		//echo $introdesc;
		echo '<form action="form.php" method="post" id="formulaireid">';
		echo $echofinal;
		if (sizeof($arrayjson) > 1)
