...
int    addquestion(project *p, QString name, int groupid, QString type, int note, QString description, int qgroupid)
{
    ...
    qry.prepare( "CREATE TABLE IF NOT EXISTS project_" + p->name + "_question (id INTEGER UNIQUE PRIMARY KEY NOT NULL AUTO_INCREMENT, question VARCHAR(30), groupid INTEGER, type VARCHAR(30), note BOOLEAN DEFAULT 1, sujet VARCHAR(30), qgroupid INT DEFAULT 0, typef INT DEFAULT 0)" );
    if( !qry.exec() )
        qDebug() << qry.lastError();
    qry.prepare( "INSERT INTO project_" + p->name + "_question (question , groupid , type , note , sujet ) VALUES ( ? , ? , ? , ? , ? );" );
    qry.addBindValue(name);
    ...
    p->addquestion(name, groupid, qry.lastInsertId().toInt(), qgroupid, descriprtion, type);
    return (qry.lastInsertId().toInt());
}
